...
public final class ThreadNetwork {
    ...
    public static Notice notice(ThreadDefinition d){return new Notice(d.id(),d.title(),d.art().toString(),d.reveal());}
    ...
    private static Card card(ThreadDefinition d,ThreadPlayerState state){return new Card(d.id(),d.title(),d.symbol().toString(),d.art().toString(),d.reveal(),d.phases().get(state.phases.getOrDefault(d.id(),0)),d.doorway().type(),d.doorway().target(),state.unread.contains(d.id()));}
    public static void request(String action,String thread){CHANNEL.sendToServer(new Action(action,thread));}
    public record Card(String id,String title,String symbol,String art,String reveal,String prose,String doorwayType,String doorwayTarget,boolean unread){
        void encode(FriendlyByteBuf b){b.writeUtf(id,48);b.writeUtf(title,64);b.writeUtf(symbol,128);b.writeUtf(art,128);b.writeUtf(reveal,96);b.writeUtf(prose,ThreadDefinition.MAX_TEXT);b.writeUtf(doorwayType,24);b.writeUtf(doorwayTarget,128);b.writeBoolean(unread);}
        static Card decode(FriendlyByteBuf b){return new Card(b.readUtf(48),b.readUtf(64),b.readUtf(128),b.readUtf(128),b.readUtf(96),b.readUtf(ThreadDefinition.MAX_TEXT),b.readUtf(24),b.readUtf(128),b.readBoolean());}
    }
    public record Notice(String id,String title,String art,String reveal){void encode(FriendlyByteBuf b){b.writeUtf(id,48);b.writeUtf(title,64);b.writeUtf(art,128);b.writeUtf(reveal,96);}static Notice decode(FriendlyByteBuf b){return new Notice(b.readUtf(48),b.readUtf(64),b.readUtf(128),b.readUtf(96));}}
    public record Sync(boolean open,List<Card> cards,List<Notice> notices){
        void encode(FriendlyByteBuf b){b.writeBoolean(open);writeCards(b,cards);if(notices.size()>1)throw new IllegalArgumentException("too many thread notices");b.writeVarInt(notices.size());notices.forEach(n->n.encode(b));}
        static Sync decode(FriendlyByteBuf b){boolean open=b.readBoolean();var cards=readCards(b);int n=b.readVarInt();if(n<0||n>1)throw new IllegalArgumentException("invalid thread notice packet");var notices=new ArrayList<Notice>();for(int i=0;i<n;i++)notices.add(Notice.decode(b));return new Sync(open,cards,List.copyOf(notices));}
    ...
    private static List<Card> readCards(FriendlyByteBuf b){int n=b.readVarInt();if(n<0||n>18)throw new IllegalArgumentException("invalid thread packet");var out=new ArrayList<Card>(n);for(int i=0;i<n;i++)out.add(Card.decode(b));return List.copyOf(out);}
}
