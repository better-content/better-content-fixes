...
public final class DynamicTreesSupportSweepCommand {
    ...
    public static void onRegisterCommands(final RegisterCommandsEvent event) {
        ...
        dispatcher.register(Commands.literal("btmfixes_dt_support_sweep")
    ...
    private static int run(final CommandSourceStack source) {
        ...
        try {
            ...
            if (failures.isEmpty()) {
                source.sendSuccess(() -> Component.literal("[BTMFIXES] DT support sweep passed for all registered species."), true);
                return 1;
            }
            source.sendFailure(Component.literal("[BTMFIXES] DT support sweep failures: " + String.join("; ", failures)));
            return 0;
        } catch (ReflectiveOperationException e) {
            source.sendFailure(Component.literal("[BTMFIXES] DT support sweep reflection failure: " + e.getMessage()));
            return 0;
        }
    }
}
